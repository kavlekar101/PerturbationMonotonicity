...
def Perturbation(df, inspector, frac):
    ...
    groupData = groupData[groupData["criticalFound"]
                          == 0].sample(frac=frac, random_state=42)
...
foodDataOrigin = pd.read_csv("food_inspec_data.csv", index_col=0, dtype={
                             "Inspector": "category"})
...
fc = cvglmnetPredict(cvfit, X_test.to_numpy().copy(),
                     ptype="response", s=cvfit["lambda_min"])
print(X_test.shape)
print(fc.shape)
